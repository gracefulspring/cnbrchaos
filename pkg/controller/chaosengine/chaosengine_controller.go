...
import (
	...
	"github.com/gracefulspring/cnbrchaos/pkg/analytics"
	cnbrchaosv1alpha1 "github.com/gracefulspring/cnbrchaos/pkg/apis/cnbrchaos/v1alpha1"
	"github.com/gracefulspring/cnbrchaos/pkg/controller/resource"
	...
	"github.com/gracefulspring/cnbrchaos/pkg/controller/watcher"
	dynamicclientset "github.com/gracefulspring/cnbrchaos/pkg/dynamic"
...
func watchChaosResources(clientSet client.Client, c controller.Controller) error {
	...
	if err := c.Watch(&source.Kind{Type: &cnbrchaosv1alpha1.ChaosEngine{}}, &handler.EnqueueRequestForObject{}); err != nil {
		return err
	}
...
func (r *ReconcileChaosEngine) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if engine.Instance.Spec.EngineState == cnbrchaosv1alpha1.EngineStateActive && engine.Instance.Status.EngineStatus == cnbrchaosv1alpha1.EngineStatusInitialized {
		return r.reconcileForCreationAndRunning(engine, reqLogger)
	}

	// Handling Graceful completion of ChaosEngine
	if engine.Instance.Spec.EngineState == cnbrchaosv1alpha1.EngineStateStop && engine.Instance.Status.EngineStatus == cnbrchaosv1alpha1.EngineStatusCompleted {
		return r.reconcileForComplete(engine, request)
	}

	// Handling forceful Abort of ChaosEngine
	if engine.Instance.Spec.EngineState == cnbrchaosv1alpha1.EngineStateStop && engine.Instance.Status.EngineStatus == cnbrchaosv1alpha1.EngineStatusInitialized {
		return r.reconcileForDelete(engine, request)
	}

	// Handling restarting of ChaosEngine post Abort
	if engine.Instance.Spec.EngineState == cnbrchaosv1alpha1.EngineStateActive && (engine.Instance.Status.EngineStatus == cnbrchaosv1alpha1.EngineStatusStopped) {
		return r.reconcileForRestartAfterAbort(engine, request)
	}

	// Handling restarting of ChaosEngine post Completion
	if engine.Instance.Spec.EngineState == cnbrchaosv1alpha1.EngineStateActive && (engine.Instance.Status.EngineStatus == cnbrchaosv1alpha1.EngineStatusCompleted) {
...
func getChaosRunnerENV(cr *cnbrchaosv1alpha1.ChaosEngine, aExList []string, ClientUUID string) []corev1.EnvVar {
...
func getChaosRunnerLabels(cr *cnbrchaosv1alpha1.ChaosEngine) map[string]string {
...
func initializeApplicationInfo(instance *cnbrchaosv1alpha1.ChaosEngine, appInfo *chaosTypes.ApplicationInfo) (*chaosTypes.ApplicationInfo, error) {
	if instance == nil {
...
func (r *ReconcileChaosEngine) reconcileForDelete(engine *chaosTypes.EngineInfo, request reconcile.Request) (reconcile.Result, error) {
	...
	if engine.Instance.ObjectMeta.Finalizers != nil {
		engine.Instance.ObjectMeta.Finalizers = utils.RemoveString(engine.Instance.ObjectMeta.Finalizers, "chaosengine.cnbrchaos.io/finalizer")
	}

	// Update ChaosEngine ExperimentStatuses, with aborted Status.
	updateExperimentStatusesForStop(engine)
	engine.Instance.Status.EngineStatus = cnbrchaosv1alpha1.EngineStatusStopped
...
func (r *ReconcileChaosEngine) updateEngineState(engine *chaosTypes.EngineInfo, state cnbrchaosv1alpha1.EngineState) error {
	patch := client.MergeFrom(engine.Instance.DeepCopy())
...
func (r *ReconcileChaosEngine) gracefullyRemoveDefaultChaosResources(engine *chaosTypes.EngineInfo, request reconcile.Request) (reconcile.Result, error) {
	if engine.Instance.Spec.JobCleanUpPolicy == cnbrchaosv1alpha1.CleanUpPolicyDelete {
...
func (r *ReconcileChaosEngine) reconcileForComplete(engine *chaosTypes.EngineInfo, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.updateEngineState(engine, cnbrchaosv1alpha1.EngineStateStop); err != nil {
...
func (r *ReconcileChaosEngine) reconcileForRestartAfterComplete(engine *chaosTypes.EngineInfo, request reconcile.Request) (reconcile.Result, error) {
	...
	engine.Instance.Status.EngineStatus = cnbrchaosv1alpha1.EngineStatusInitialized
...
func (r *ReconcileChaosEngine) initEngine(engine *chaosTypes.EngineInfo) error {
	if engine.Instance.Spec.EngineState == "" {
		engine.Instance.Spec.EngineState = cnbrchaosv1alpha1.EngineStateActive
	}

	if engine.Instance.Spec.EngineState == cnbrchaosv1alpha1.EngineStateActive && engine.Instance.Status.EngineStatus == "" {
		engine.Instance.Status.EngineStatus = cnbrchaosv1alpha1.EngineStatusInitialized
	}

	if engine.Instance.Status.EngineStatus == cnbrchaosv1alpha1.EngineStatusInitialized {
		if engine.Instance.ObjectMeta.Finalizers == nil {
...
func (r *ReconcileChaosEngine) reconcileForCreationAndRunning(engine *chaosTypes.EngineInfo, reqLogger logr.Logger) (reconcile.Result, error) {
	if err := r.validateAnnontatedApplication(engine); err != nil {
		if stopEngineWithAnnotationErrorMessage := r.updateEngineState(engine, cnbrchaosv1alpha1.EngineStateStop); stopEngineWithAnnotationErrorMessage != nil {
			r.recorder.Eventf(engine.Instance, corev1.EventTypeWarning, "ChaosResourcesOperationFailed", "(chaos stop) Unable to update chaosengine")
			return reconcile.Result{}, fmt.Errorf("unable to Update Engine State: %v", err)
		}
		return reconcile.Result{}, err
...
func updateExperimentStatusesForStop(engine *chaosTypes.EngineInfo) {
	for i := range engine.Instance.Status.Experiments {
		if engine.Instance.Status.Experiments[i].Status == cnbrchaosv1alpha1.ExperimentStatusRunning || engine.Instance.Status.Experiments[i].Status == cnbrchaosv1alpha1.ExperimentStatusWaiting {
			engine.Instance.Status.Experiments[i].Status = cnbrchaosv1alpha1.ExperimentStatusAborted
...
func (r *ReconcileChaosEngine) updateEngineForComplete(engine *chaosTypes.EngineInfo, isCompleted bool) error {
	if engine.Instance.Status.EngineStatus != cnbrchaosv1alpha1.EngineStatusCompleted {
		engine.Instance.Status.EngineStatus = cnbrchaosv1alpha1.EngineStatusCompleted
		engine.Instance.Spec.EngineState = cnbrchaosv1alpha1.EngineStateStop
		if err := r.client.Update(context.TODO(), engine.Instance, &client.UpdateOptions{}); err != nil {
...
func (r *ReconcileChaosEngine) updateChaosResult(engine *chaosTypes.EngineInfo, request reconcile.Request) error {
	chaosresultList := &cnbrchaosv1alpha1.ChaosResultList{}
	opts := []client.ListOption{
		client.InNamespace(request.NamespacedName.Namespace),
...
func getChaosStatus(result cnbrchaosv1alpha1.ChaosResult) ([]cnbrchaosv1alpha1.TargetDetails, map[string]string) {
	...
	for k, v := range annotations {
		...
		case "injected", "reverted", "targeted":
			...
			if !updateTargets(name, v, &targetsList) {
				targetsList = append(targetsList, cnbrchaosv1alpha1.TargetDetails{
					Name:        name,
					Kind:        kind,
					ChaosStatus: v,
				})
...
func updateTargets(name, status string, data *[]cnbrchaosv1alpha1.TargetDetails) bool {
	for i := range *data {
		if (*data)[i].Name == name {
